...
const std::vector<std::string> AnimScripter<dim>::animScriptTypeStrs = {
    "null", "grasp", "scaleF", "hang", "hang2", "hangTopLeft", "hangLeft", "swing",
    ...
    case AST_GRASP: {
        ...
        if (curTime > DBCTimeRange[0] && curTime <= DBCTimeRange[1]) {
            ...
            for (const auto& DBCInfoI : mesh.DBCInfo) {
                ...
                // if (curTime <= 0.2) {
                //     for (const auto& vI : DBCInfoI.first) {
                //         searchDir.template segment<dim>(vI * dim) = mesh.V.row(vI).transpose() - mesh.V.row(vI).transpose();
                //     }
                // } else {
                    for (const auto& vI : DBCInfoI.first) {
                        searchDir.template segment<dim>(vI * dim)
...
template <int dim>
double AnimScripter<dim>::getCurtime(void)
{
    return curTime;
}
