...
int Optimizer<dim>::solve(int maxIter)
{
    ...
    for (int iterI = 0; iterI < maxIter; iterI++) {
        ...
        if (!toggle) {
            ...
            double contact0, contact1; contact0 = contact1 = 0;
            energyTerms[0]->getEnergyValPerElemBySVD(result, false, svd, F, energyValPerElem, false);
            for (int vI = result.componentNodeRange[1]; vI < result.componentNodeRange[2]; ++vI) {
                contact0 += energyValPerElem(vI);
            }
            for (int vI = result.componentNodeRange[2]; vI < result.componentNodeRange[3]; ++vI) {
                contact1 += energyValPerElem(vI);
            }
            spdlog::debug("Contact0: {:f}, Contact1: {:f}", contact0, contact1);
            if (contact0 > 5e4 && contact1 > 5e4) {
                spdlog::debug("Max contact energy reached; stop closing, start lifting.");
                for (auto& DBCInfoI : result.DBCInfo) {
                    DBCInfoI.second[0](0) = 0;
                    DBCInfoI.second[0](1) = 2.5;
                    DBCInfoI.second[0](2) = 0;
                }
		for (auto& scriptedLVelI : result.componentLVels) {
                    scriptedLVelI.second(0) = 0;
                    scriptedLVelI.second(1) = 2.5;
                    scriptedLVelI.second(2) = 0;
		}
                toggle = true;
            } 
        }
...
void Optimizer<dim>::saveStatus(const std::string& appendStr)
{
    ...
    igl::writeOBJ(outputFolderPath + "test_" + std::to_string(globalIterNum) + ".obj", V_surf, F_surf);
